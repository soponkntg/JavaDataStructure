public class HuffmanNode implements Comparable {
	...
	public static HuffmanNode buildHuffmanTree(Heap h) throws Exception {
		...
		while((h.size() != 1)) {
			HuffmanNode firstPop = (HuffmanNode) h.pop();
			HuffmanNode secondPop = (HuffmanNode) h.pop();
			int sum = firstPop.frequency + secondPop.frequency;
			HuffmanNode parent;
			if(firstPop.frequency < secondPop.frequency) {
				parent = new HuffmanNode(sum, ' ', firstPop, secondPop);
			}else {
				parent = new HuffmanNode(sum, ' ', secondPop, firstPop);
			}
			h.add(parent);
		}
		//default return
