...
public class BST {
	...
	public void cloneTree(BST tt) {
		...
		while(data.size() > 0) {
			...
			if(t.currentNode.left != null) {
				data.add(t.currentNode.left.data);
				insert(t.currentNode.left.data);
			}
			if(t.currentNode.right != null) {
				data.add(t.currentNode.right.data);
				insert(t.currentNode.right.data);
			}
		}
	...
	public int findNextData(int n) throws Exception {
		// Implement this method 
		if(isEmpty()) return n;
		if(n < findMin(root).currentNode.data) return findMin(root).currentNode.data;
		TreeIterator itr = findMin(root);
		while(itr.hasNext()) {
			if(n >= itr.currentNode.data && n <itr.next()) {
